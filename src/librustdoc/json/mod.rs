...
fn target(sess: &rustc_session::Session) -> types::Target {
    ...
    let feature_stability: FxHashMap<&str, Stability<_>> = sess
        .target
    ...
    types::Target {
        ...
        target_features: sess
            ...
            .filter(|(_, stability, _)| {
                ...
                stability.toggle_allowed(&sess.target, true).is_ok()
            })
            .map(|(name, stability, implied_features)| {
                types::TargetFeature {
                    name: name.into(),
                    unstable_feature_gate: match stability {
                        Stability::Unstable { nightly_feature: feature_gate, .. } => {
                            Some(feature_gate.as_str().into())
                        }
                        _ => None,
                    },
